...
* Deploy the apps by using the deployment manifests: `oc apply -f deployment-temperature-sensor-1.yaml`, `oc apply -f deployment-temperature-sensor-2.yaml` and `oc apply -f deployment-thermostat.yaml`. This will start two temperature sensors and one thermostat system.
...
oc apply -f deployment-thermostat.yaml
```
